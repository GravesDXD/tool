var SlimScroll = (function () {
  ...
  $.prototype = {
    ...
    closest: function (selector) {
      ...
      while (parent.el !== $(selector).el && parent.el !== $("html").el) {
  ...
  function SlimScroll(el, options) {
    ...
    function _onWheel(e) {
      ...
      const showDialog = document.querySelector('.el-dialog__wrapper:not([style*="display: none"]), .el-message-box__wrapper:not([style*="display: none"])');
      if (showDialog) {
        if (!showDialog.querySelector('.' + o.wrapperClass)) {
          return;
        }
      }
      var delta = 0;
    ...
    //触发全局滚动事件
    //使用popover组件，POPover无法监听到滚动事件，定位出现问题
    function _triggerGlobalScrollEvent(target) {
      //创建一个event
      var event = document.createEvent('HTMLEvents');

      // 初始化even，并命名为'resize'.
      //因为popover偶尔会监听不到scroll事件，所以改为resize
    ...
    function scrollContent(y, isWheel, isJump) {
      ...
      var toolTipDOM = document.getElementsByClassName('el-tooltip__popper');
...
//       var o = binding.value;
//       if (o.height !== 'auto') {
//         var wrapper = document.getElementsByClassName(o.wrapperClass)[0];
